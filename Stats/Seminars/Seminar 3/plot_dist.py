...
def plot_est_hist_normal(error_gen, estimator, mu, sigma, sample_sizes, num_repeats, figsize=(20, 20), x_min=None, x_max=None, y_max=None):
    """
    ...
    for i, (n, ax)  in enumerate(zip(sample_sizes, axes.flatten()[:k])):
        errors = error_gen(loc=0, scale=sigma, size=(num_repeats, n))
        Y = mu + errors
        mY = estimator(Y)
        ax.hist(mY, bins=100, alpha=0.2, label=f'Meadian on {n} samples', density=True, color=cmap(i))
